...
class GoogleSheetsService:
        ...
        for current_date in sorted_dates:
            ...
            while current_time < end_time:
                ...
                if booking:
                    ...
                    booking_duration_slots = booking.duration_minutes // self.project_config.slot_duration_minutes
                    for i in range(booking_duration_slots):
                        if i == 0:
                            ...
                            dash_row = ["", current_date.strftime("%d.%m.%Y"), 
                                       (current_time + timedelta(minutes=self.project_config.slot_duration_minutes*i)).strftime("%H:%M"),
                                       "-", "-", "-"]
                            worksheet.update(f'A{row}:F{row}', [dash_row])
                        row += 1
                    
                    # Skip the additional slots in the loop
                    current_time += timedelta(minutes=self.project_config.slot_duration_minutes* booking_duration_slots)
                else:
                    # Empty slot
                    row_data = [
                        current_date.strftime("%d.%m") if first_row_of_day else "",  # A
                    ...
                    worksheet.update(f'A{row}:F{row}', [row_data])
                    row += 1
                    current_time += timedelta(minutes=self.project_config.slot_duration_minutes)
    ...
    def get_available_slots(self, db: Session, target_date: date, time_fraction: int) -> AvailableSlots:
        ...
        for specialist in self.project_config.specialists:
            ...
            # Проверяем на ошибку листа
            if sheets_reserved == ["WORKSHEET_ERROR"]:
                logger.error(f"Worksheet error for {specialist}, marking all slots as unavailable")
                # Помечаем что произошла ошибка с листом
                all_slots[f"available_slots_{specialist.lower()}"] = ["ERROR: Лист специалиста не найден"]
                continue
    ...
    def _get_all_work_slots_for_specialist(
        ...
        while current_time + timedelta(minutes=self.project_config.slot_duration_minutes* effective_time_fraction) <= end_time:
            slot_time = current_time.time()
    ...
    def _get_available_slots_for_specialist(
        ...
        for booking in bookings:
            ...
            booking_duration_slots = booking.duration_minutes // self.project_config.slot_duration_minutes
            for i in range(booking_duration_slots):
                slot_time = (booking_time + timedelta(minutes=self.project_config.slot_duration_minutes*i)).time()
        ...
        while current_time + timedelta(minutes=self.project_config.slot_duration_minutes* effective_time_fraction) <= end_time:
            ...
            for i in range(effective_time_fraction):
                check_time = (current_time + timedelta(minutes=self.project_config.slot_duration_minutes*i)).time()
    ...
    def _get_reserved_slots_for_specialist(
        ...
        if time_fraction <= 0:
            ...
            while current_time < end_time:
                ...
                if current_time + timedelta(minutes=self.project_config.slot_duration_minutes* time_fraction) > end_time:
                ...
                for i in range(time_fraction):
                    ...
                    if check_time in occupied_slots:
                        ...
                        for j in range(time_fraction):
                            conflict_start = current_time - timedelta(minutes=self.project_config.slot_duration_minutes*j)
                            if conflict_start.time() >= work_start and conflict_start < current_time + timedelta(minutes=self.project_config.slot_duration_minutes):
    ...
    def _get_reserved_slots_from_sheets(self, specialist_name: str, target_date: date, time_fraction: int) -> List[str]:
        ...
        try:
            ...
            try:
                ...
                return ["WORKSHEET_ERROR"]  # Специальный маркер ошибки
            ...
            try:
                ...
                for row_idx, row in enumerate(all_values[1:], start=2):  # Skip header row
                    if len(row) >= 3:  # Changed from 7 to 3 - need at least date and time  # Ensure we have all columns (A-G)
                    ...
                    for client_id, sheets_bookings in client_bookings_in_sheets.items():
                        ...
                        for db_booking in db_bookings:
                            ...
                            if not found_in_sheets:
                                # Не отменяем записи младше 60 секунд - дать время на синхронизацию с Sheets
                                from datetime import datetime, timedelta
                                if db_booking.created_at < datetime.utcnow() - timedelta(seconds=60):
                                    logger.info(f"Deactivating booking not found in sheets: {client_id} - {booking_time}")
                                    db_booking.status = "cancelled"
                                else:
                                    logger.info(f"Skipping deactivation for fresh booking: {client_id} - {booking_time}")
                                db.commit()
                finally:
                    db.close()
    ...
    def _is_slot_available_with_time_fraction(self, slot_time: str, reserved_slots: List[str], time_fraction: int) -> bool:
        ...
        try:
            ...
            for i in range(time_fraction):
                check_time = (slot_datetime + timedelta(minutes=self.project_config.slot_duration_minutes* i)).strftime("%H:%M")
    ...
    def _get_available_slots_for_specialist_in_time_range(
        ...
        while current_time + timedelta(minutes=self.project_config.slot_duration_minutes* effective_time_fraction) <= end_datetime:
            slot_time = current_time.time()
    ...
    def update_single_booking_slot(self, specialist_name: str, booking: Booking) -> bool:
        ...
        try:
            ...
            if target_row:
                ...
                duration_slots = booking.duration_minutes // self.project_config.slot_duration_minutes
                if duration_slots > 1:
    ...
    def is_slot_available_in_sheets(self, specialist_name: str, booking_date: date, booking_time: time) -> bool:
        ...
        try:
            # Проверяем в БД кэше
            query = text("""
                SELECT client_id, client_name, service 
                FROM sheets_slots 
            ...
            if result:
                # Слот занят если client_id не пустой (включая прочерки)
                is_occupied = (
                    result.client_id is not None and 
                    result.client_id != '' and 
                    result.client_id.strip() != ''
                )
                
                is_available = not is_occupied
                
                logger.debug(f"Slot {booking_time} on {booking_date}: client_id='{result.client_id}', available={is_available}")
                return is_available
            else:
                logger.warning(f"No slot found in DB for {specialist_name} {booking_date} {booking_time}")
                return False
                
        except Exception as e:
            logger.error(f"Error checking slot availability in DB: {e}", exc_info=True)
            return False
        finally:
            db.close()
    ...
    async def add_booking_to_make_table_async(self, booking_data: dict) -> bool:
        ...
        try:
            ...
            # Get current Unix timestamp for row creation time
            ...
            row_data = [
                ...
                creation_timestamp  # K: Unix timestamp of row creation time
            ]
            
            # Append to sheet
            worksheet.append_row(row_data)
            logger.info(f"Successfully added booking to Make.com table with appointment timestamp {unix_timestamp}, creation timestamp {creation_timestamp} and status flags")
            return True
    ...
    async def delete_booking_from_make_table(self, client_id: str, date: str, time: str) -> bool:
        """
        Удаляет запись из таблицы Make.com при отмене или переносе
        
        Args:
            client_id: Messenger ID клиента (Telegram ID)
            date: Дата записи в формате DD.MM.YYYY
            time: Время записи в формате HH:MM
        """
        try:
            logger.info(f"Deleting booking from Make.com table: client={client_id}, date={date}, time={time}")
            ...
            for idx, row in enumerate(all_rows, start=1):
                if len(row) >= 8:
                    # row[0] - дата, row[1] - время, row[7] - messenger_id
                    if row[0] == date and row[1] == time and row[7] == client_id:
                        row_to_delete = idx
                        logger.info(f"Found row to delete: {idx}")
                        break
            
            if row_to_delete:
                worksheet.delete_rows(row_to_delete)
                logger.info(f"Successfully deleted row {row_to_delete} from Make.com table")
                return True
            else:
                logger.warning(f"No matching row found in Make.com table for deletion")
                return False
                
        except Exception as e:
            logger.error(f"Failed to delete booking from Make.com table: {e}")
            return False
    ...
    async def update_booking_status_in_main_table(self, specialist_name: str, date: str, time: str, status: str) -> bool:
        """
        Updates booking confirmation status in the main specialist table (column H)
        ...
        """
        try:
            logger.info(f"Updating booking status in main table for {specialist_name} on {date} {time} to {status}")
            
            if not self.spreadsheet:
                logger.warning("Cannot update booking status: no spreadsheet connection")
                return False
            ...
            try:
                ...
                logger.error(f"Worksheet not found for specialist: {specialist_name}")
                return False
            
            # Find the row with matching date and time
            target_row = self._find_row_for_time_slot(
                worksheet,
            ...
            if target_row:
                ...
                worksheet.update(f'H{target_row}', [[status_text]])
                logger.info(f"Updated status in main table row {target_row} column H to: {status_text}")
                return True
            else:
                logger.warning(f"Could not find booking for {specialist_name} at {date} {time}")
                return False
                
        except Exception as e:
            logger.error(f"Failed to update booking status in main table: {e}")
            return False


    async def log_error_to_sheets_async(self, error_data: dict) -> bool:
        """Логирует ошибку в отдельный лист 'Ошибки' в таблице Make.com"""
        try:
            # Используем project_config как в других методах
            make_sheet_id = getattr(self.project_config, 'google_sheet_make_id', None)
            if not make_sheet_id:
                # Пробуем из переменных окружения как запасной вариант
                import os
                make_sheet_id = os.getenv('GOOGLE_SHEET_MAKE_ID')
                
            if not make_sheet_id:
                logger.warning("Make.com sheet ID not configured for error logging")
                return False
            
            gc = self._get_sheets_client()
            spreadsheet = gc.open_by_key(make_sheet_id)
            
            # Создаем или получаем лист "Ошибки"
            try:
                worksheet = spreadsheet.worksheet("Ошибки")
                logger.info("Found existing 'Ошибки' worksheet")
            except Exception as worksheet_error:
                logger.info(f"Creating new 'Ошибки' worksheet: {worksheet_error}")
                try:
                    worksheet = spreadsheet.add_worksheet(title="Ошибки", rows=1000, cols=10)
                    logger.info("Successfully created 'Ошибки' worksheet")
                except Exception as create_error:
                    logger.error(f"Failed to create 'Ошибки' worksheet: {create_error}")
                    raise
                # Добавляем заголовки
                headers = ["Дата/Время", "ID клиента", "Имя клиента", "Тип ошибки", "Сообщение об ошибке", "Message ID", "Статус"]
                worksheet.append_row(headers)
            ...
            row_data = [
                ...
                str(error_data.get('client_id', '')),
                error_data.get('client_name', 'Unknown'),
                error_data.get('error_type', 'booking_error'),
            ...
            worksheet.append_row(row_data)
            logger.info(f"Error logged to sheets: {error_data.get('error_message', '')[:50]}")
            return True
